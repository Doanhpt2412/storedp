<x-layouts.admin title="Thêm Chuyên mục mới">
    <div class="mb-6 flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Thêm Chuyên mục mới</h1>
        </div>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden max-w-3xl">
        <form action="{{ route('admin.post-categories.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" value="{{ old('slug') }}"
                        class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm px-4 py-2 border transition-colors bg-slate-50 @error('slug') border-red-500 @enderror" placeholder="De trong de tu dong tao">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm px-4 py-2 border transition-colors @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
